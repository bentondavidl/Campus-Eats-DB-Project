...
SET @@GLOBAL.GTID_PURGED=/*!80000 '+'*/ '5275787a-1f8c-11eb-8e84-42010a8e0054:1-606868';
...
--
-- Temporary view structure for view `delivery_history`
--

DROP TABLE IF EXISTS `delivery_history`;
/*!50001 DROP VIEW IF EXISTS `delivery_history`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `delivery_history` AS SELECT 
 1 AS `driver_id`,
 1 AS `order_id`,
 1 AS `make`,
 1 AS `model`,
...
--
-- Final view structure for view `delivery_history`
--
...
/*!50001 VIEW `delivery_history` AS select `order`.`driver_id` AS `driver_id`,`order`.`order_id` AS `order_id`,`vehicle`.`make` AS `make`,`vehicle`.`model` AS `model`,`delivery`.`delivery_time` AS `delivery_time`,`restaurant`.`restaurant_name` AS `restaurant_name`,`order`.`total_price` AS `total_price`,(select avg(`d`.`stars`) from `driver_rating` `d` where ((`d`.`driver_id` = `order`.`driver_id`) and (`d`.`person_id` = `order`.`person_id`))) AS `rating` from (((`order` join `delivery` on((`delivery`.`delivery_id` = `order`.`delivery_id`))) join `vehicle` on((`vehicle`.`vehicle_id` = `delivery`.`vehicle_id`))) join `restaurant` on((`restaurant`.`restaurant_id` = `order`.`restaurant_id`))) order by `order`.`driver_id`,`order`.`total_price` desc */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `order_history` AS select `order`.`person_id` AS `person_id`,`location`.`location_name` AS `location`,`person`.`person_name` AS `driver_name`,(select avg(`sub`.`stars`) AS `avg_stars` from `driver_rating` `sub` where ((`sub`.`driver_id` = `driver`.`driver_id`) and (`order`.`person_id` = `sub`.`person_id`))) AS `driver_rating`,`restaurant`.`restaurant_name` AS `restaurant_name`,(select avg(`sub`.`stars`) AS `avg_stars` from `restaurant_rating` `sub` where ((`sub`.`restaurant_id` = `restaurant`.`restaurant_id`) and (`order`.`person_id` = `sub`.`person_id`))) AS `restaurant_rating`,`order`.`total_price` AS `total_price` from (((((`order` join `location` on((`order`.`location_id` = `location`.`location_id`))) join `driver` on((`order`.`driver_id` = `driver`.`driver_id`))) join `student` on((`driver`.`student_id` = `student`.`student_id`))) join `person` on((`student`.`person_id` = `person`.`person_id`))) join `restaurant` on((`order`.`restaurant_id` = `restaurant`.`restaurant_id`))) order by `order`.`person_id` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2020-12-10 16:58:32
